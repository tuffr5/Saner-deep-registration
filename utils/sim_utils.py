...
"""Contain SSIM, SSD, NCC, MI, MIND, local MI
    Adopted from https://github.com/junyuchen245/
"""
...
class ClassDiceLoss(nn.Module):
    """Dice and entropy loss"""

    def __init__(self, num_class=36):
        super().__init__()
        self.num_class = num_class

    def one_hot(self, x, i):
        x = -torch.abs(x - i) + 1
        return F.relu(x)
    ...
    def forward(self, y_pred, y_true):
        ...
        unique_class = torch.arange(1, self.num_class).to("cuda:0")
        dice = []
        for i in unique_class:
            tmp_pred = self.one_hot(y_pred.clone(), i)
            tmp_true = self.one_hot(y_true.clone(), i)

            intersection = torch.sum(tmp_pred * tmp_true)
            union = torch.pow(tmp_pred, 2).sum() + torch.pow(tmp_true, 2).sum()
            dice.append((2. * intersection) / (union + 1e-5))

        dice = torch.hstack(dice)
