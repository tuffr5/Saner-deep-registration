"""
    OASIS Dataset Image Registration
...
def seg_dsp_one_hot(x_seg, flow, img_warper):
    x_seg_oh = F.one_hot(x_seg.long(), num_classes=36)
...
@torch.no_grad()
def argmax_interpolation(x_seg, flow, img_warper, num_classes=36):
...
def sanity_checks_test(register, img_warper, data, loss_bundle, flag='val'):
    ...
    if flag == 'test':
        def_seg = img_warper(mov_seg.float(), dsp_m2f, 'nearest')
    ...
    def_seg = img_warper(mov_seg.float(), dsp_m2f, 'nearest')
    mdice = loss_bundle.dice_score(def_seg.long(), fix_seg.long())

    def_seg = img_warper(fix_seg.float(), dsp_f2m, 'nearest')
    fdice = loss_bundle.dice_score(def_seg.long(), mov_seg.long())

    def_seg = img_warper(mov_seg.float(), dsp_m2m, 'nearest')
    self_dice = loss_bundle.dice_score(def_seg.long(), mov_seg.long())

    def_seg = img_warper(fix_seg.float(), dsp_f2f, 'nearest')
    self_dice += loss_bundle.dice_score(def_seg.long(), fix_seg.long())

    self_dice = self_dice / 2
    # else:
    #     # Note using argmax_interpolation function dice can be higher, but to be consistent to TransMorph paper, we don't use here.
...
def sanity_checks_train(register, img_warper, data, loss_bundle, opts, optimizer):
    ...
    def_seg = seg_dsp_one_hot(mov_seg, dsp_m2f, img_warper)
    loss_dice = loss_bundle.dice_loss(def_seg, fix_seg)

    # grad_loss
    loss_grad = loss_bundle.spatial_grad(dsp_m2f)

    loss['loss_sim'] = loss_sim.item()
    loss['loss_dice'] = loss_dice.item()
    loss['loss_grad'] = loss_grad.item()

    losses = loss_sim + loss_dice + loss_grad

    optimizer.zero_grad(set_to_none=True)
    losses.backward()
    optimizer.step()

    del losses, loss_sim, loss_dice, loss_grad, def_seg, dsp_m2f

    dsp_f2m = register(fix, mov)
    loss_sim = loss_bundle.dist_loss(mov, img_warper(fix, dsp_f2m))

    def_seg = seg_dsp_one_hot(fix_seg, dsp_f2m, img_warper)
    loss_dice = loss_bundle.dice_loss(def_seg, mov_seg)
    ...
    del losses, loss_sim, loss_dice, loss_grad, def_seg, dsp_f2m

    # cross_sanity
    if opts.flag_cross:
        loss_cross = loss_bundle.cross_sanity_loss(register(mov, fix), register(fix, mov))
        loss['loss_cross'] = loss_cross.item()
        if loss_cross > 1e-5:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_type", type=str, default="transmorph-large", help='Model type')
    parser.add_argument("--model_dir", type=str,
                        default="/data/duanbin/image_registration/experiments/OASIS/TransMorph-Large/",
    ...
    parser.add_argument("--beta", type=float, default=10, help="Beta weight for cross-sanity check")
    ...
    opts.train_dir = join(opts.data_root, 'OASIS_data', 'All/')

    opts.val_dir = join(opts.data_root, 'OASIS_data', 'Test/')
    ...
    if not opts.testing:
        ...
        train_set = datasets.OASISBrainDataset(glob.glob(opts.train_dir + '*.pkl'), transforms=train_composed)

        train_loader = DataLoader(train_set, batch_size=1, shuffle=True, num_workers=4, pin_memory=True)

        val_composed = transforms.Compose([trans.NumpyType((np.float32, np.int16))])
        val_set = datasets.OASISBrainInferDataset(glob.glob(opts.val_dir + '*.pkl'), transforms=val_composed)
        ...
        test_set = datasets.OASISBrainInferDataset(glob.glob(opts.val_dir + '*.pkl'), transforms=test_composed)
